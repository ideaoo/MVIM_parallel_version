...
for x in range(0,1):
    globals()['path_'+str(x)] = "C:/Users/smcmlab-24/Desktop/test5/temp_400K_%s.dump"%(x)
    
    # the folder of the final files
    globals()['path'+str(x)] = "C:/Users/smcmlab-24/Desktop/test5"
...
# open the files of the references and the targets 
    for id in range(pieces):

        locals()['fdata'+str(id)] = open(globals()['path'+str(x)]+'/file_%s.dump'%(id), 'w')
        locals()['ref'+str(id)] = open(globals()['path'+str(x)]+'/ref_%s.dump'%(id), 'w')
